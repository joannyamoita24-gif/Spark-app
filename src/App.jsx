...
const sampleProfiles = [
  { id: 1, name: "Amara", age: 26, job: "Fashion Designer", distance: 2, gender: "woman", ethnicity: "Black", bio: "Nairobi born, world traveler 🌍 Love art, good food and deep talks.", tags: ["Fashion", "Travel", "Art"], color: "#f87171", emoji: "🌺" },
  { id: 2, name: "James", age: 29, job: "Chef", distance: 5, gender: "man", ethnicity: "Mixed", bio: "Half Irish, half Nigerian 🍀🌍 I cook jollof AND shepherd's pie.", tags: ["Cooking", "Music", "Food"], color: "#60a5fa", emoji: "👨‍🍳" },
  { id: 3, name: "Priya", age: 24, job: "Software Engineer", distance: 1, gender: "woman", ethnicity: "Asian", bio: "Indian girl who loves Afrobeats 💃 Hiker by day, Netflix by night.", tags: ["Tech", "Hiking", "Music"], color: "#a78bfa", emoji: "✨" },
  { id: 4, name: "Lucas", age: 31, job: "Photographer", distance: 8, gender: "man", ethnicity: "Latino", bio: "Brazilian living in Nairobi 📷 I shoot portraits and fall for smiles.", tags: ["Photography", "Travel", "Art"], color: "#34d399", emoji: "📸" },
  { id: 5, name: "Zara", age: 22, job: "Doctor", distance: 3, gender: "woman", ethnicity: "Mixed", bio: "British-Somali 🇬🇧 Saving lives and dancing to Burna Boy on weekends 💃", tags: ["Dancing", "Fitness", "Adventure"], color: "#fb923c", emoji: "🌻" },
  { id: 6, name: "Ethan", age: 28, job: "Musician", distance: 6, gender: "man", ethnicity: "White", bio: "White boy who grew up in Mombasa 🎵 I play guitar and speak Swahili fluently.", tags: ["Music", "Culture", "Beach"], color: "#f472b6", emoji: "🎸" },
  { id: 7, name: "Yuki", age: 25, job: "Architect", distance: 4, gender: "woman", ethnicity: "Asian", bio: "Japanese-Kenyan 🇯🇵🇰🇪 I design buildings and make the best sushi in Nairobi.", tags: ["Design", "Food", "Art"], color: "#38bdf8", emoji: "🏛️" },
  { id: 8, name: "David", age: 33, job: "Teacher", distance: 9, gender: "man", ethnicity: "White", bio: "American in Nairobi for love of the continent ❤️ Coaching football on weekends.", tags: ["Sports", "Education", "Culture"], color: "#4ade80", emoji: "🏈" },
  { id: 9, name: "Nia", age: 27, job: "Lawyer", distance: 2, gender: "woman", ethnicity: "Black", bio: "Lagos to Nairobi pipeline 💼 I argue for fun and dance for free 🕺", tags: ["Law", "Dance", "Foodie"], color: "#e879f9", emoji: "👩‍⚖️" },
  { id: 10, name: "Carlos", age: 30, job: "Entrepreneur", distance: 7, gender: "man", ethnicity: "Latino", bio: "Mexican-American building businesses in Africa 🚀 Salsa dancing is my therapy.", tags: ["Business", "Salsa", "Travel"], color: "#facc15", emoji: "🌮" },
];
    ...
    <div style={S.page}>
      <div style={{ width: "100%", maxWidth: 480, background: "white", padding: "16px 24px 0", boxShadow: "0 2px 20px rgba(0,0,0,0.06)", position: "sticky", top: 0, zIndex: 10 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <span style={{ fontSize: 13, color: "#aaa" }}>Hi, {user.name} 👋</span>
      ...
      <div style={{ width: "100%", maxWidth: 480, padding: "20px 16px", flex: 1 }}>
        {tab === "discover" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div style={{ color: "#bbb", fontSize: 13 }}>❤️ {liked} liked • {filtered.length} nearby</div>
              <button onClick={() => setShowFilters(f => !f)} style={{ background: showFilters?"#e84393":"#fff0f6", border: "none", borderRadius: 20, padding: "6px 14px", fontSize: 13, color: showFilters?"white":"#e84393", cursor: "pointer", fontWeight: 600 }}>⚙️ Filters</button>
            </div>
            ...
            {showFilters && (
              <div style={{ background: "white", borderRadius: 16, padding: 20, marginBottom: 16, boxShadow: "0 4px 20px rgba(232,67,147,0.1)", border: "1px solid #f0e6f0" }}>
                ...
                <label style={S.label}>🎂 Age Range: <strong style={{ color: "#e84393" }}>{minAge} – {maxAge} yrs</strong></label>
                <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 11, color: "#aaa", marginBottom: 4 }}>Min age</div>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 11, color: "#aaa", marginBottom: 4 }}>Max age</div>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
                  {["everyone", "men", "women"].map(g => (
                    <button key={g} onClick={() => { setGenderFilter(g); setCardIndex(0); }} style={{ flex: 1, padding: "8px 0", borderRadius: 20, border: `2px solid ${genderFilter===g?"#e84393":"#f0e6f0"}`, background: genderFilter===g?"#e84393":"white", color: genderFilter===g?"white":"#888", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "Georgia,serif", textTransform: "capitalize" }}>{g}</button>
                  ))}
                </div>
                <label style={S.label}>🌍 Ethnicity</label>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {["All","Black","White","Asian","Latino","Mixed"].map(e => (
                    <button key={e} onClick={() => { setEthnicityFilter(e); setCardIndex(0); }} style={{ padding: "7px 14px", borderRadius: 20, border: `2px solid ${ethnicityFilter===e?"#e84393":"#f0e6f0"}`, background: ethnicityFilter===e?"#e84393":"white", color: ethnicityFilter===e?"white":"#888", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "Georgia,serif" }}>{e}</button>
                  ))}
                </div>
              </div>
            )}
            ...
            {filtered.length === 0 ? (
              ...
              <>
                <div style={{ position: "relative", height: 440, marginBottom: 20 }}>
                  ...
                  <div style={{ position: "absolute", inset: 0, borderRadius: 24, background: `linear-gradient(160deg,white 40%,${current?.color}33)`, boxShadow: `0 20px 60px ${current?.color}40`, border: `2px solid ${current?.color}55`, padding: 28, display: "flex", flexDirection: "column", justifyContent: "space-between", transform: swipeDir==="right"?"translateX(120%) rotate(20deg)":swipeDir==="left"?"translateX(-120%) rotate(-20deg)":"none", transition: swipeDir?"transform 0.4s ease":"none" }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 26, fontWeight: 800, color: "#1a1a2e" }}>{current?.name}, {current?.age}</div>
                ...
                <div style={{ display: "flex", justifyContent: "center", gap: 20, alignItems: "center" }}>
                  <button onClick={() => swipe("left")} style={{ width: 70, height: 70, borderRadius: "50%", border: "2px solid #fca5a5", background: "white", fontSize: 28, cursor: "pointer", boxShadow: "0 4px 20px rgba(239,68,68,0.2)" }}>✕</button>
                  ...
                  <button onClick={() => swipe("right")} style={{ width: 70, height: 70, borderRadius: "50%", border: "2px solid #bbf7d0", background: "white", fontSize: 28, cursor: "pointer", boxShadow: "0 4px 20px rgba(34,197,94,0.2)" }}>⭐</button>
                </div>
              </>
            )}
          </div>
          ...
          <div>
            ...
            {sampleProfiles.slice(0, 4).map(m => (
              <div key={m.id} onClick={() => { setActiveMatch(m); setTab("chat"); }} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 16px", background: "white", borderRadius: 16, cursor: "pointer", boxShadow: `0 2px 16px ${m.color}25`, border: `1px solid ${m.color}33`, marginBottom: 10 }}>
                <div style={{ width: 52, height: 52, borderRadius: "50%", background: `linear-gradient(135deg,${m.color}66,${m.color})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24 }}>{m.emoji}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 800, color: "#1a1a2e", fontSize: 15 }}>{m.name}, {m.age}</div>
                  <div style={{ fontSize: 12, color: "#e84393", fontWeight: 600 }}>🌍 {m.ethnicity}</div>
                  <div style={{ fontSize: 13, color: "#888", marginTop: 2 }}>📍 {m.distance} km away</div>
                </div>
        ...
        {tab === "profile" && (
          <div>
            <div style={{ textAlign: "center", marginBottom: 24 }}>
              <div style={{ width: 90, height: 90, borderRadius: "50%", background: "linear-gradient(135deg,#e84393,#f97316)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 44, margin: "0 auto 12px" }}>😊</div>
            ...
            <div style={{ background: "white", borderRadius: 16, padding: 20, boxShadow: "0 2px 16px rgba(0,0,0,0.06)", marginBottom: 16 }}>
              ...
              {[["Age", user.age||"Not set"],["Job", user.job||"Not set"],["Bio", user.bio||"Not set"]].map(([k,v]) => (
                <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: "1px solid #f5f5f5" }}>
                  ...
                  <span style={{ color: "#1a1a2e", fontSize: 14, fontWeight: 600 }}>{v}</span>
                </div>
              ))}
            </div>
